...
object tweet {
  ...
  prop.setProperty("user", "demo")
  prop.setProperty("password", "demo")
  ...
  case class Tweet(Id:Long ,createdAt:Long, source:String,text:String)
  ...
  */
    ...
    //val words: tweets[String] = ...
      ...
      //Tweet(status.getId,status.getCreatedAt().getTime()/1000,status.getSource(), status.getText(),status.getGeoLocation.toString())).foreachRDD { rdd =>

    tweets.map(status=>
      Tweet(status.getId,status.getCreatedAt().getTime()/1000,status.getSource(), status.getText())).foreachRDD { rdd =>
      ...
      wordCountsDataFrame.write.mode("append").jdbc(url, "tweetstable",prop)
    }
